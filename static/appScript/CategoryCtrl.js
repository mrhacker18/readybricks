...
//    console.log($scope.totalpaging);
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'Category_ctrl/'+action,
			  data: data,
 ...
 CategoryCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
	$scope.fgShowHide=true;
...
CategoryCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
CategoryCtrl.prototype.searchPopup=function($scope){
