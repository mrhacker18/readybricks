...
//			$scope.list=res.data;
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'Roles_ctrl/'+action,
			  data: data,
 ...
 RolesCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
...
RolesCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
    ...
    $scope.gridOptions = { 
        ...
        columnDefs: [
				{field:'', displayName:'Action', width:actionWidth,	cellTemplate:'<div style="position:relative;top:4px;padding-left:2px"><button ng-show="auth.update"  ng-click="editItem(row)" class="btn btn-primary btn-mini" ><i class="icon-edit icon-white"></i> Edit</button>&nbsp;<button ng-show="auth.delete" ng-click="deleteItem(row)" class="btn btn-danger btn-mini"><i class="icon-trash icon-white"></i> Delete</button> </div>'
								...
								,{field: 'RoleName', displayName: 'RoleName'}
				,{field: 'Navigations_NavName', displayName: 'Navigations'}
				,{field: 'IsRead', displayName: 'IsRead'}
				,{field: 'IsInsert', displayName: 'IsInsert'}
				,{field: 'IsUpdate', displayName: 'IsUpdate'}
				,{field: 'IsDelete', displayName: 'IsDelete'}
...
RolesCtrl.prototype.searchPopup=function($scope){
