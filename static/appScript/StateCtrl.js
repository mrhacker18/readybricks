...
//    console.log($scope.totalpaging);
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'State_ctrl/'+action,
			  data: data,
 ...
 StateCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
	$scope.fgShowHide=true;
...
StateCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
StateCtrl.prototype.searchPopup=function($scope){
	$scope.showForm=function(){$scope.fgShowHide=false; $scope.state=null;};
