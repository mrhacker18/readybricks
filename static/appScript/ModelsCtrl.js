...
function ModelsCtrl($scope, $http){	
	...
	$scope.saveItem=function(){	
		...
		if($scope.item==null || $scope.item=="" ){
            ...
            $scope.errors.nameMsg = 'Please enter model name.';
            return false;
        }
...
//    console.log($scope.totalpaging);
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'Models_ctrl/'+action,
			  data: data,
 ...
 ModelsCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
	$scope.fgShowHide=true;
...
ModelsCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
ModelsCtrl.prototype.searchPopup=function($scope){
