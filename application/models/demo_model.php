...
class Demo_model extends CI_Model
{
    ...
    {
        return $this->db->select('DId,PartyName,Address,Price,AppTime,PhoneNo,DateTime,Remark,Status,models.ModelId,models.Name as ModelName,pump.PumpId,pump.Name as PumpName') //,oMenuId,Qty,Price,SubTotal,Tax,GrandTotal // 1 pending 2 approved 3 processing  4 ready for devliery 5 deliveryed
    ...
    {
        return $this->db->select('DId,PartyName,demo.Address,Price,AppTime,PhoneNo,AltPhoneNo,CPersonName,DateTime,Remark,demo.Status,models.ModelId,OrderBy,models.Name as ModelName,pump.PumpId,pump.Name as PumpName,users.FirstName,users.LastName') //,oMenuId,Qty,Price,SubTotal,Tax,GrandTotal // 1 pending 2 approved 3 processing  4 ready for devliery 5 deliveryed
		->join('models', 'models.ModelId = demo.ModelId', 'inner')
		->join('pump', 'pump.PumpId = demo.PumpId', 'inner')
		->join('users', 'users.UserId = demo.OrderBy', 'inner')
		->where('demo.Status', $status)
		->order_by('DateTime', 'ASC')->get($this->table)->result();
    }
    public function get_payment_status_all($status)
    {

         $this->db->select('OrderId,CustId,customer.Name as CustomerName,PhoneNo,orders.Address,orders.PaymentCollection,orders.oUserId,orders.MainType,orders.SolutionType,orders.PaymentMode,orders.OrderBy,orders.SerialNo,orders.Remark,orders.OrderType,orders.PaymentReceivedBy,orders.Status,orders.DateTime') //,oMenuId,Qty,Price,SubTotal,Tax,GrandTotal // 1 pending 2 approved 3 processing  4 ready for devliery 5 deliveryed
    ...
    {
        return $this->db->select('OrderId,orders.OUserId,orders.ModelId,orders.SerialNo,CustId,customer.Name as CustomerName,PhoneNo,orders.Address,orders.PaymentCollection,orders.PaymentMode,orders.OrderBy,orders.Remark,orders.OrderType,orders.MainType,orders.Status,orders.DateTime,models.Name as ModelName,pump.PumpId,pump.Name as PumpName') //,oMenuId,Qty,Price,SubTotal,Tax,GrandTotal // 1 pending 2 approved 3 processing  4 ready for devliery 5 deliveryed
		->join('customer', 'OCustId = CustId', 'inner')
		->join('models', 'models.ModelId = orders.ModelId', 'inner')
		->join('pump', 'pump.PumpId = orders.PumpId', 'inner')
...
");
	...
	public function get_page_where_maintenance($size, $pageno, $params){
		$this->db->limit($size, $pageno)
			->select('OrderId,CustId,customer.Name as CustomerName,SerialNo,PhoneNo,orders.Address,orders.PaymentCollection,orders.DateTime,orders.OrderBy,orders.Remark,orders.OrderType,orders.Status,models.ModelId,models.Name as ModelName,pump.PumpId,pump.Name as PumpName')
//			->join('users', 'oUserId = UserId', 'left outer');
	...
	$this->db->select('orders.OrderId,orders.SerialNo,orders.OUserId,orders.ModelId,orders.DateTime,orders.BuildingType,orders.OrderBy,customer.CustId,customer.Name,customer.PhoneNo,orders.Address as OrderAddress,pump.PumpId, pump.Name as PumpName')
