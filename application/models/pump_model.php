...
class Pump_model extends CI_Model
{
	...
	public function get_page($size, $pageno){
		$this->db
			->limit($size, $pageno)
			->select('pump.PumpId,pump.Name,pump.Status');
	...
	public function get_page_where($size, $pageno, $params){
		...
		if(isset($params->search) && !empty($params->search)){
				$this->db->where("name LIKE '%$params->search%'");
...
// ->join('Navigations', 'Roles.NavigationId = Navigations.NavigationId', 'left outer');
    ...
    {
        return $this->db->where('PumpId', $id)->get($this->table)->row();
    }
    ...
    {
        return $this->db->where('PumpId', $id)->update($this->table, $data);
    }

    public function delete($id)
    {
        $this->db->where('PumpId', $id)->delete($this->table);
